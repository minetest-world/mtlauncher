	...
	{#if content}
		<div class="px-64">
			{#if installedMods.length}
				<div class="pb-16">
					<h1 class="text-2xl font-bold pb-4">{$_('mods.installed_mods')}</h1>
					<div class="grid grid-cols-2 gap-4">
						{#each installedMods as item}
							<div class="flex h-48 contentblock">
								<img src="{item.thumbnail}" alt="Thumbnail" class="backgroundimg" />
								<button type="button" on:click={() => goto(`/mods/${item.slug}`)} class="flex flex-col w-full p-4 bg-black/50 h-full justify-end hover:bg-black/60 hover:cursor-pointer">
			...
			{/if}
			<h1 class="text-2xl font-bold pb-4">{$_('mods.all_mods')}</h1>
			<div class="grid grid-cols-2 gap-4">
